...
const AddUsers: React.FC<AddUsersProps> = ({ onAdd }) => {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<AdminUserFormValue>({ mode: "onBlur" });
  const { close } = useModalContext();

  const [data, setData] = useState<AdminUserFormValue>({
    username: "",
    firstName: "",
    lastName: "",
      ...
      <Box style={{ padding: "20px" }}>
        ...
        {errors.userContactInfo?.contactPhone && (
          <p className="error-msg">
            {errors.userContactInfo.contactPhone.message}
          </p>
        )}
        ...
        <select
          ...
          value={data.role?.id || ""}
          onChange={(e) =>
            handleDataChange({ role: { id: parseInt(e.target.value) } })
          }
          style={{ marginTop: "20px", color: data.role?.id ? "black" : "gray" }}
        >
        ...
        <Button
          ...
          style={{ backgroundColor: "rgb(214, 129, 1)", marginTop: "20px" }}
          fullWidth
          variant="contained"
        >
          Submit
