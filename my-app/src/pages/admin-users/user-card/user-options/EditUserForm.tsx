...
const EditUserForm: React.FC<EditUserFormProps> = ({ user, onEdit }) => {
    ...
    const handleRoleChange = (partialData: Partial<Role>) => {
        ...
        if (selectedRole) {
            setUserInfo({ ...userInfo, role: { id: selectedRole.id, role: selectedRole.role } })
        }
    }
    ...
    return (
        <div style={{ padding: '15px' }}>
            ...
            <TextField
                ...
                onChange={(e) => handleRoleChange({ role: e.target.value as keyof typeof UserRoles })}
            ...
            <Button onClick={handleSubmit} style={{ backgroundColor: 'rgb(214, 129, 1)', marginTop: '20px', marginBottom: '20px' }} fullWidth variant="contained" color="primary">
                Submit
            </Button>
        </div>
    )
