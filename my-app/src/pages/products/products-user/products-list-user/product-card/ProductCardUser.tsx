...
const ProductCardUser: React.FC<ProductCardUserProps> = ({ product }) => {
    ...
    const { orderProducts, addProduct, increaseQuantity } = useUserContext();

    const handleAdd = () => {
        const searchedProducts = orderProducts.find((p) => p.product.id === product.id);
        if (searchedProducts) {
    ...
    return (
        <div className="product-card">
            ...
            <div className="product-option">
                <button className='product-option-btn only-edit' onClick={handleAdd}>ADD TO ORDER</button>
            </div>
        </div>
    )
}
